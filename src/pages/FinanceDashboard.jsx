...
export default function FinanceDashboard() {
  ...
  const handleExport = () => {
    ...
    const csvRows = [
      headers.join(","), 
      ...data.map(d => `"${d.Project}","${d.Budget}","${d.Cost}","${d.Status}"`) // Wrap in quotes
    ];
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8 relative z-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
          <SummaryCard title="Total Budget" value="$310,000" note="+8% from last quarter" color="green" />
          <SummaryCard title="Total Cost" value="$290,000" note="–5% under budget" color="red" />
          <SummaryCard title="Projects" value="4 Active" note="2 new this month" color="yellow" />
          <SummaryCard title="Variance" value="+$12,950" note="Positive trend" color="green" />
        </div>
